   ...
   contains
      ...
      module subroutine omp_device_memcpy_int8(array_dst, array_src, omp_dev, lbounds, ubounds)
         ...
         rank(1)
            ...
            !$omp target device(omp_dev) has_device_addr(array_dst, array_src) map(to:lbounds_, ubounds_)
#else
            !$omp target device(omp_dev) is_device_ptr(array_dst, array_src) map(to:lbounds_, ubounds_)
#endif
